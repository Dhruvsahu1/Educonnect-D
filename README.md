
# 🎓 EduConnect

### Secure Cloud-Native Social & Academic Platform

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain-wordmark.svg" height="80" alt="Kubernetes" /> &nbsp;
<img src="https://k3s.io/img/k3s-logo-light.svg" height="40" alt="K3s" /> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" height="60" alt="Google Cloud" /> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/githubactions/githubactions-original.svg" height="60" alt="GitHub Actions" /> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/terraform/terraform-original.svg" height="60" alt="Terraform" /> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="60" alt="Docker" />

[![SonarQube Cloud](https://sonarcloud.io/images/project_badges/sonarcloud-light.svg)](https://sonarcloud.io/summary/new_code?id=Dhruvsahu1_Educonnect-D)

A Cloud-Native Platform that unifies student collaboration, academic content management, and secure software delivery through DevSecOps, Kubernetes, and Zero-Trust Security principles.

---

[![🦂 ZAP Security](https://github.com/Dhruvsahu1/Educonnect-D/actions/workflows/dast.yaml/badge.svg)](https://github.com/Dhruvsahu1/Educonnect-D/actions/workflows/dast.yaml)
[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/Dhruvsahu1/Educonnect-D/badge)](https://securityscorecards.dev/viewer/?uri=github.com/Dhruvsahu1/Educonnect-D)
[![CI Pipeline](https://github.com/Dhruvsahu1/Educonnect-D/actions/workflows/ci.yml/badge.svg)](https://github.com/Dhruvsahu1/Educonnect-D/actions/workflows/ci.yml)

![CodeQL](https://img.shields.io/badge/Security-CodeQL-blueviolet?logo=github)
![SonarCloud](https://img.shields.io/badge/SonarCloud-Quality_Gate-success?logo=sonarcloud)
![Trivy](https://img.shields.io/badge/Trivy-Vulnerability_Scanning-blue?logo=aquasecurity)
![CycloneDX](https://img.shields.io/badge/SBOM-CycloneDX-informational)

[![Verified by InfraScan](https://img.shields.io/badge/Verified_by-SolDevelo_InfraScan-0052cc?style=flat&logo=security)](https://github.com/soldevelo/infrascan)

---



# 🚀 Overview



EduConnect is a cloud-native academic and social networking platform designed to bridge the gap between students and educational institutions.



Students can showcase certifications, engage in discussions, collaborate academically, and access study resources, while administrators manage institutions, moderate content, and distribute educational materials.



The platform is engineered using modern DevSecOps practices and deployed on Kubernetes with security embedded throughout the software delivery lifecycle.



---



# 🎥 Architecture Walkthrough



<p align="center">

<a href="https://youtu.be/gALR-TCQo9M?si=cjIpVR47RVeR6sNn">

<img src="https://img.shields.io/badge/▶_Watch_Architecture_Explainer-YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white"/>

</a>

</p>



---



# ✍️ Technical Deep-Dive Articles



<table>

<tr>

<td width="50%">



### 🛡️ Cloud Native Security & K3s



<a href="https://hardik0811arora.hashnode.dev/educonnect-a-deep-dive-into-cloud-native-security-k3s-orchestration">

<img src="https://img.shields.io/badge/Read_on-Hashnode-2962FF?style=for-the-badge&logo=hashnode&logoColor=white"/>

</a>



Deep dive into:



* Kubernetes Security

* K3s Architecture

* DevSecOps Integration

* Production Hardening

* Cloud Native Deployment



</td>



<td width="50%">



### ☸️ MongoDB & K3s Production Journey



<a href="https://hardik-arora.hashnode.dev/from-localhost-to-production-my-journey-debugging-mongodb-and-nodejs-on-k3s">

<img src="https://img.shields.io/badge/Read_on-Hashnode-2962FF?style=for-the-badge&logo=hashnode&logoColor=white"/>

</a>



Lessons learned while:



* Migrating to Kubernetes

* Running MongoDB StatefulSets

* Troubleshooting Networking

* Production Debugging

* Containerized Deployments



</td>

</tr>

</table>



---



# 🏗️ Architecture


<img width="1900" height="1266" alt="Image" src="https://github.com/user-attachments/assets/65a0db0b-c2be-4696-aca7-2144a9bf6773" />

---



# 🛠️ Technology Stack



## Frontend



<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redux/redux-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="45"/>

<img src="https://vitejs.dev/logo.svg" height="45"/>

</p>



* React

* Redux Toolkit

* JavaScript

* Vite

* TailwindCSS

* shadcn/ui



---



## Backend



<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" height="45"/>

</p>



* Node.js

* Express.js

* MongoDB

* JWT Authentication

* AWS S3



---



## DevSecOps & Cloud



<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/githubactions/githubactions-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" height="45"/>

<img src="https://k3s.io/img/k3s-logo-light.svg" height="35"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/terraform/terraform-original.svg" height="45"/>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" height="45"/>

</p>



* GitHub Actions

* Docker

* Kubernetes

* K3s

* Terraform

* AWS



---



# 🔐 Security Controls


| Category            | Implementation               |
| ------------------- | ---------------------------- |
| Authentication      | JWT + Refresh Token Rotation |
| API Security        | Helmet + Rate Limiting       |
| SAST                | CodeQL                       |
| Code Quality        | SonarCloud                   |
| Container Security  | Trivy                        |
| Dependency Security | GitHub Advisory Database     |
| DAST                | OWASP ZAP                    |
| SBOM                | CycloneDX                    |
| Secrets Management  | GitHub Secrets               |
| Security Headers    | Helmet.js                    |
| Supply Chain        | OpenSSF Scorecard            |



---



# ⭐ Key Features



### 👨‍🎓 Student Portal



* Authentication & Profile Management

* Social Feed

* Certification Showcase

* Nested Comments & Discussions

* Personalized Dashboard

* Study Material Access



### 🏫 Admin Portal



* College Management

* Content Moderation

* Student Verification

* Material Distribution

* Academic Resource Management



---



# 📁 Repository Structure



```text

EduConnect

│
├── frontend
├── backend
├── infrastructure
├── kubernetes
├── scripts
└── .github/workflows

```





---











<div align="center">



### ⭐ Built with Cloud Native, DevSecOps, and Security-First Engineering


<p align="center">
<img width="64" height="64" alt="Scorpion" src="https://github.com/user-attachments/assets/c6e5289d-7152-49ab-a563-6bfd04bf375a" />
</p>


</div>
